...
export const useFocusStoreV5 = defineStore('focusV5', () => {
  ...
  const todayFocusTime = computed(() => {
    ...
    if (currentMonth === expectedMonth && calendarData.value.check_in_days.includes(todayDate)) {
      // 如果今天有签到，返回今天的专注时间
      // 这里简化处理，实际应该从API获取今天的具体专注时间
      return Math.floor(calendarData.value.total_focus_minutes / calendarData.value.check_in_days.length)
    }
  ...
  const loadFocusRecords = async (reset = false): Promise<void> => {
    try {
      ...
      if (response.data) {
        ...
        recordsPagination.value.total = response.data.total
        recordsPagination.value.hasMore = focusRecords.value.length < response.data.total
